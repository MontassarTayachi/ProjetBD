...
export default function Personnel() {
  ...
  const validateForm = () => {
    ...
    if (activeTab === 'employeurs' && !formData.nomemployeur) {
      newErrors.nomemployeur = 'Employer name is required';
    }
    
    if (activeTab === 'formateurs' && !formData.type) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (currentItem) {
        ...
        switch (activeTab) {
          case 'employeurs':
            response = await personnelService.updateEmployeur(currentItem.id, formData);
            break;
          case 'formateurs':
            response = await personnelService.updateFormateur(currentItem.id, formData);
            break;
          case 'participants':
            response = await personnelService.updateParticipant(currentItem.id, formData);
            console.log(formData)
            break;
        }
      } else {
    ...
    switch (activeTab) {
          ...
          <div>
            <div className="mb-4">
              ...
              <input
                ...
                className={`mt-1 block w-full rounded-md border ${errors.nomemployeur ? 'border-red-500' : 'border-gray-300'} shadow-sm p-2`}
              />
              {errors.nomemployeur && <p className="mt-1 text-sm text-red-600">{errors.nomemployeur}</p>}
            </div>
          </div>
        );
      case 'formateurs':
          ...
          <div>
            <div className="mb-4">
              ...
              <select
                ...
                value={formData.type}
                onChange={handleInputChange}
                className={`mt-1 block w-full rounded-md border ${errors.type ? 'border-red-500' : 'border-gray-300'} shadow-sm p-2`}
              >
      ...
      case 'participants':
        return (
          <div>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700">Formations (IDs comma separated)</label>
              <select
                name="formations"
              ...
              >
                ...
                {formations.map(emp => (
                  <option key={emp.id} value={emp.id}>{emp.titre}</option>
                ))}
              </select>
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg shadow-xl transform transition-all sm:max-w-lg w-full p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-[#7a6699]">
                  {currentItem ? `Edit ${activeTab.slice(0, -1)}` : `Add New ${activeTab.slice(0, -1)}`}
                </h2>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={formData.nom}
                            onChange={handleInputChange}
                            className={`mt-1 block w-full rounded-md border ${errors.nom ? 'border-red-500' : 'border-gray-300'} shadow-sm p-2`}
                          />
